...
#    print(x_struc.shape)
    ...
    x_train, x_1, x_train_struc, x_2, y_train, y_1 = train_test_split(x_tensor, x_struc, y_tensor,test_size=0.4, random_state=42)
    x_valid, x_test, x_valid_struc, x_test_struc, y_valid, y_test = train_test_split(x_1, x_2, y_1,test_size=0.5, random_state=42)

    struc_size = x_valid_struc.shape[1]
    ...
    for Mode in Mode_list:
        if Mode:
            ...
            test_MergeModel(torch.tensor(x_train).cuda(), torch.tensor(x_train_struc).cuda(),
                            ...
                            classifier)
            test_MergeModel(torch.tensor(x_valid).cuda(), torch.tensor(x_valid_struc).cuda(),
                            torch.tensor(y_valid).cuda(),
                            classifier)
            metric_list.append(test_MergeModel(torch.tensor(x_test).cuda(), torch.tensor(x_test_struc).cuda(),
            ...
            test_Model(torch.tensor(x_valid).cuda(), torch.tensor(y_valid).cuda(), classifier)
            metric_list.append(test_Model(torch.tensor(x_test).cuda(), torch.tensor(y_test).cuda(), classifier))
            if save_model:
    ...
    print(predict(lr, x_valid_struc, np.argmax(y_valid,axis=1)))
    print(predict(lr, x_test_struc, np.argmax(y_test,axis=1)))
    metric_list.append(predict(lr, x_test_struc, np.argmax(y_test,axis=1)))
    ...
    test_Model(torch.tensor(x_valid_struc).cuda(), torch.tensor(y_valid).cuda(), classifier)
    metric_list.append(test_Model(torch.tensor(x_test_struc).cuda(), torch.tensor(y_test).cuda(), classifier))
    if save_model:
